...
def configure():
	...
	flags.DEFINE_string('pretrain_file', '/ais/gobi6/domainadapt/datasets/ckpt/resnet_v2_101.ckpt', 'pre-trained model filename corresponding to encoder_name')
	flags.DEFINE_string('data_list', './dataset_cityscapes/train.txt', 'training data list filename')

	# validation
	flags.DEFINE_integer('valid_step', 20000, 'checkpoint number for validation')
	flags.DEFINE_integer('valid_num_steps', 500, '= number of validation samples')
	flags.DEFINE_string('valid_data_list', './dataset_cityscapes/val_fine.txt', 'validation data list filename')
	...
	flags.DEFINE_integer('test_num_steps', 1525, '= number of testing/validation samples')
	flags.DEFINE_string('test_data_list', './dataset_cityscapes/test.txt', 'testing/validation data list filename')
	flags.DEFINE_boolean('visual', True, 'whether to save predictions for visualization')

	# data
	flags.DEFINE_string('data_dir', '/ais/gobi6/domainadapt/datasets/cityscapes', 'data directory')
	flags.DEFINE_integer('batch_size', 10, 'training batch size')
	flags.DEFINE_integer('input_height', 1024, 'input image height')
	flags.DEFINE_integer('input_width', 2048, 'input image width')
